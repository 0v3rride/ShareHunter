...
def getShareComments(comment):
    try:
        return " : {}".format(comment) if comment is not None else "";
    except Exception as er:
        return "";

def enumShares(hostsFile, ports, user, pword):
    with io.open(hostsFile, "r") as hosts:
        for host in hosts.read().splitlines():
            for port in ports:
                try:
                    s = SMBConnection(user, pword, gethostname(), host, use_ntlm_v2=True, sign_options=SMBConnection.SIGN_WHEN_REQUIRED, is_direct_tcp=True);
                    s.connect(host, port);
                    ...
                    print("MESSAGE: {} - {}\n".format(host, e));
                    #print("Connection to {} on port {} could not be made...\nThe port may not be open, the host may be down or doesn't exist or credentials are required to access the shares on this host!\n".format(host, port));


def enumShare(host, ports, user, pword):
    for port in ports:
        try:
            s = SMBConnection(user, pword, gethostname(), host, use_ntlm_v2=True, sign_options=SMBConnection.SIGN_WHEN_REQUIRED, is_direct_tcp=True);
            s.connect(host, port);
...
def parseArgs():
    ...
    if(args.host and not args.hosts):
        enumShare(args.host, args.ports, args.user, args.pword);
    elif(args.hosts and not args.host):
        enumShares(path.abspath(args.hosts), args.ports, args.user, args.pword);
    else:
